...
def run(episodes, is_training=True, render=False, save_video=False):
    # Initialize the LunarLander environment
    env = gym.make("LunarLander-v3", continuous=False, render_mode='rgb_array' if save_video else ('human' if render else None))
    ...
    video_frames = []  # For storing successful landing frames
    ...
    for episode in tqdm(range(episodes), desc="Running Episodes"):
        ...
        while not done:
            ...
            if done and reward > 0:
                ...
                if save_video:
                    video_frames.append(env.render())  # Save frames of successful landing
    ...
    # Save video of successful landings
    if save_video and video_frames:
        import imageio
        imageio.mimsave("successful_landings_training.mp4", video_frames, fps=30)
...
if __name__ == "__main__":
    # Train the agent
    run(5000, is_training=True, render=False, save_video=True)

    # Evaluate the agent
    run(50, is_training=False, render=True, save_video=True)
